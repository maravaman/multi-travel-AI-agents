...
class BaseAgent(ABC):
    ...
    def store_interaction(self, user_id: int, query: str, response: str, 
        ...
        """
        try:
            # Store in short-term memory (Redis) - graceful fallback
            if self.memory and hasattr(self.memory, 'set_stm'):
                self.memory.set_stm(user_id, self.name, query)
            
            # Store in long-term memory (MySQL) - graceful fallback
            if self.memory and hasattr(self.memory, 'store_ltm') and self.memory.mysql_available:
                try:
                    self.memory.store_ltm(user_id, self.name, query, response)
                except Exception as ltm_error:
                    logger.debug(f"LTM storage failed, continuing without it: {ltm_error}")
            
            # Store interaction with metadata if memory supports it
            if self.memory and hasattr(self.memory, 'store_interaction'):
                try:
                    ...
                    logger.debug(f"Interaction storage failed, continuing: {interaction_error}")
            
            logger.debug(f"{self.name} stored interaction for user {user_id}")
            
        except Exception as e:
            logger.debug(f"Memory storage unavailable for {self.name}, continuing without it: {e}")
    ...
    def get_recent_interactions(self, user_id: int, hours: int = 2) -> List[Dict]:
        ...
        """
        try:
            if self.memory and hasattr(self.memory, 'get_recent_stm') and self.memory.redis_available:
                return self.memory.get_recent_stm(user_id, self.name, hours)
            else:
                return []  # Graceful fallback
        except Exception as e:
            logger.debug(f"Failed to get recent interactions for {self.name}, using fallback: {e}")
            return []
    ...
    def get_historical_context(self, user_id: int, days: int = 7) -> List[Dict]:
        ...
        """
        try:
            if self.memory and hasattr(self.memory, 'get_ltm_by_agent') and self.memory.mysql_available:
                return self.memory.get_ltm_by_agent(user_id, self.name)
            else:
                return []  # Graceful fallback
        except Exception as e:
            logger.debug(f"Failed to get historical context for {self.name}, using fallback: {e}")
            return []
    ...
    def generate_response_with_context(self, query: str, context: str, 
        ...
        """
        Generate response using intelligent fallback (no slow LLM calls)
        ...
        """
        try:
            # Use immediate intelligent response instead of slow Ollama
            from api.main import generate_travel_response
            response = generate_travel_response(query)
            return response
                
        except Exception as e:
            logger.debug(f"Intelligent response generation for {self.name}: {e}")
            # Ultimate fallback
            return f"I'm your {self.name} assistant. For your query '{query}', I'm here to help with expert travel guidance. Let me provide you with personalized recommendations and actionable advice!"
