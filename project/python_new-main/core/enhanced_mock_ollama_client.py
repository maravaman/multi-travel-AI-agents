...
class EnhancedMockOllamaClient:
    ...
    def generate_response(
        ...
        # Travel-related responses (high priority)
        if any(word in prompt_lower for word in ["trip", "travel", "vacation", "journey", "plan", "visit", "tokyo", "japan", "europe", "flight", "hotel", "itinerary", "budget"]):
            return self._generate_travel_response(prompt, agent_type, context)
        
        # Weather-related responses
        elif any(word in prompt_lower for word in ["weather", "temperature", "rain", "sun", "climate", "forecast", "storm", "snow"]):
    ...
    def _generate_travel_response(self, prompt: str, agent_type: str, context: List[str] = None) -> str:
        """Generate travel-specific mock response"""
        destination = self._extract_destination(prompt)
        destination_text = f" to {destination}" if destination else ""
        
        return f"""🎯 {agent_type} Travel Planning:

I'll help you plan an amazing trip{destination_text}! Here's a comprehensive approach:

**🗺️ Trip Planning Framework:**
• **Destination Research**: Culture, attractions, and local customs
• **Budget Planning**: Transportation, accommodation, food, and activities  
• **Timing Strategy**: Best seasons, weather patterns, and local events
• **Logistics Coordination**: Flights, hotels, transportation, and reservations

**📋 Essential Planning Steps:**
1. **Define Your Goals**: Relaxation, adventure, culture, or business?
2. **Set Realistic Budget**: Include buffer for unexpected expenses
3. **Research Destinations**: Climate, culture, and key attractions
4. **Plan Itinerary**: Balance must-sees with spontaneous discovery
5. **Handle Logistics**: Book major items, keep flexibility for details

**💡 Pro Travel Tips:**
• Book flights and accommodation first for better rates
• Leave 30% of your itinerary unplanned for discoveries
• Research local customs and basic phrases
• Consider travel insurance for international trips
• Pack essentials but buy locally when possible

**🎯 Next Actions:**
• Clarify your travel dates and duration
• Define your primary interests (culture, food, nature, etc.)
• Set a realistic budget range
• Research visa/documentation requirements

**Budget Planning Guide:**
• Transportation: 25-30% of total budget
• Accommodation: 25-30% of total budget  
• Food & Dining: 20-25% of total budget
• Activities & Attractions: 15-20% of total budget
• Buffer & Miscellaneous: 10-15% of total budget

What specific aspects would you like me to focus on for your trip planning?"""
    
    def _extract_destination(self, prompt: str) -> str:
        """Extract destination from travel prompt"""
        prompt_lower = prompt.lower()
        destinations = {
            "japan": "Japan", "tokyo": "Tokyo, Japan", "kyoto": "Kyoto, Japan",
            "europe": "Europe", "paris": "Paris, France", "london": "London, UK",
            "italy": "Italy", "rome": "Rome, Italy", "barcelona": "Barcelona, Spain",
            "amsterdam": "Amsterdam, Netherlands", "berlin": "Berlin, Germany",
            "thailand": "Thailand", "bangkok": "Bangkok, Thailand",
            "bali": "Bali, Indonesia", "australia": "Australia", "sydney": "Sydney, Australia"
        }
